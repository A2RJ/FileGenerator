...
if (is_dir($dir) == true && is_dir($dirT) == true) {
    ...
    $tbody .= '
        <td class="text-center">
            <a href="/produk/edit/<?= $p["' . $rows[0] . '"]; ?>" class="btn btn-warning">Edit</a>
            <a href="/produk/detail<?= $p["' . $rows[0] . '"]; ?>" class="btn btn-success">Detail</a>
            <form action="/produk/<?= $p["' . $rows[0] . '"]; ?>" method="post" class="d-inline">
                <?= csrf_field(); ?>
